...
module TClass = struct
	let get_member_fields' acc self_too c0 tl =
	...
	let get_all_super_fields c =
		get_member_fields' PMap.empty false c (List.map snd c.cl_params)

	let get_all_fields c tl =
		get_member_fields' (PMap.map (fun cf -> c,cf) c.cl_fields) true c tl
end