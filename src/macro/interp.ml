		...
		"make_expr", Fun2 (fun v p ->
			...
			| VAbstract (APos p) ->
				...
				let rec loop = function
					...
					| VAbstract (APos p) ->
						(Ast.EObjectDecl (
							(("fileName",Globals.null_pos),false,(Ast.EConst (Ast.String p.Globals.pfile) , p)) ::
							(("lineNumber",Globals.null_pos),false,(Ast.EConst (Ast.Int (string_of_int (Lexer.get_error_line p))),p)) ::
							(("className",Globals.null_pos),false,(Ast.EConst (Ast.String ("")),p)) ::
							[]
						), p)
					...
					| VObject o as v ->
						...
						| None ->
							...
							| Some (VAbstract (ATDecl t)) ->
								make_path t
							| _ ->
								let fields = List.fold_left (fun acc (fid,v) -> ((field_name ctx fid,Globals.null_pos),false,loop v) :: acc) [] (Array.to_list o.ofields) in
	...
	let rec loop (e,p) =
		let tag, pl = match e with
			...
			| EParenthesis e ->
				4, [loop e]
			| EObjectDecl fl ->
				5, [enc_array (List.map (fun ((f,p),quoted,e) -> enc_obj [
					"field",enc_string f;
