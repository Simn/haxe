						...
						| _ ->
							...
							match cl.cl_params with
								...
								| _ ->
									...
									in
									if cfs_must_be_native (cl.cl_structure()).cl_ordered_fields then begin
										cl.cl_meta <- (Meta.NativeGeneric, [], cl.cl_pos) :: cl.cl_meta;
										Some false
		...
		let rec run md =
			match md with
				...
				| TClassDecl ({ cl_params = hd :: tl } as cl) when set_hxgeneric gen md ->
					...
					cs'.cl_fields <- PMap.add name iface_cf cs'.cl_fields;
					...
					List.iter (fun f -> cs'.cl_fields <- PMap.add f.cf_name f cs'.cl_fields) fields;
