...
let mk_infos ctx p params =
	...
	(EObjectDecl (
		(("fileName",null_pos) , (EConst (String(file,false)) , p)) ::
		(("lineNumber",null_pos) , (EConst (Int (string_of_int (Lexer.get_error_line p))),p)) ::
		(("className",null_pos) , (EConst (String (s_type_path ctx.curclass.cl_path,false)),p)) ::
		if ctx.curfield.cf_name = "" then
			params
		else
			(("methodName",null_pos), (EConst (String(ctx.curfield.cf_name,false)),p)) :: params
	) ,p)
...
and type_field ?(resume=false) ctx e i p mode =
		...
		let rec loop_dyn c params =
			...
			| Some t ->
				...
				if (mode = MGet || mode = MCall) && PMap.mem "resolve" c.cl_fields then begin
					...
					AKExpr (make_call ctx (mk (TField (e,FInstance (c,params,f))) tfield p) [Codegen.type_constant ctx.com (String(i,false)) p] t p)
				end else
	...
	| TAbstract (a,pl) ->
		...
		with Not_found -> try
			...
			AKExpr ((!build_call_ref) ctx (AKUsing(ef,c,cf,e)) [EConst (String(i,false)),p] NoValue p)
		with Not_found ->
