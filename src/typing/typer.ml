...
let mk_infos ctx p params =
	...
	(EObjectDecl (
		(("fileName",null_pos),false,(EConst (String file) , p)) ::
		(("lineNumber",null_pos),false,(EConst (Int (string_of_int (Lexer.get_error_line p))),p)) ::
		(("className",null_pos),false,(EConst (String (s_type_path ctx.curclass.cl_path)),p)) ::
		if ctx.curfield.cf_name = "" then
			params
		else
			(("methodName",null_pos),false,(EConst (String ctx.curfield.cf_name),p)) :: params
	) ,p)
...
and type_object_decl ctx fl with_type p =
	...
	let type_fields field_map =
		...
		let fl = List.map (fun ((n,pn),is_quoted,e) ->
			let is_valid = not is_quoted || Parser.is_valid n in
	...
	(match a with
	| ODKPlain ->
		let rec loop (l,acc) ((f,pf),is_quoted,e) =
			let is_valid = not is_quoted || Parser.is_valid f in
...
and type_call ctx e el (with_type:with_type) p =
	...
	| (EConst (Ident "trace"),p) , e :: el ->
		...
		let params = (match el with [] -> [] | _ -> [("customParams",null_pos),false,(EArrayDecl (List.map mk_to_string_meta el) , p)]) in
