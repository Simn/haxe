	...
	let rec make pctx toplevel t e =
		...
		let rec loop e = match fst e with
			...
			| EBinop(OpAssign,e1,e2) ->
				let rec loop dko e = match e with
					| (EConst (Ident s),p) ->
						...
						| None -> ()
						| Some dk -> ignore(TyperDisplay.display_expr ctx (TyperBase.TypeableExpr.create_default e) (mk (TLocal v) v.v_type p) dk (WithType.with_type t) p);
						end;
			...
			| EDisplay((EConst (Ident _),_ as e),dk) when pctx.ctx.com.display.dms_kind = DMDefault ->
				...
				let locals' = ctx.locals in
				ctx.locals <- locals;
				ignore(TyperDisplay.handle_edisplay ctx (TyperBase.TypeableExpr.create_default e) (DKPattern toplevel) (WithType.with_type t));
				ctx.locals <- locals';
				pat
